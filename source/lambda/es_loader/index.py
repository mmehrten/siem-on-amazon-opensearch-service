...
def output_metrics(metrics, excluded_log_count, counted_log_count, s3obj_size, s3key, total_log_count, logtype, collected_metrics={}, **kwargs):
...
def main(event, context):
    if 'Records' in event:
        ...
        if event_source == 'aws:s3':
            ...
            for record in event['Records']:
                ...
                if data["messageType"] == "DATA_MESSAGE":
                    ...
                    for e in data["logEvents"]:
                        ...
                        total_metrics["is_ignored"] = total_metrics["is_ignored"] or log.is_ignored
            ...
            log_metrics(total_metrics["error_logs_count"], total_metrics["is_ignored"], total_metrics["ignored_reason"], collected_metrics, error_reason_list)
            output_metrics(
                metrics,
...
def process_record(record, publish: bool = True):
    ...
    if publish:
        ...
        log_metrics(logfile.error_logs_count, logfile.is_ignored, logfile.ignored_reason, collected_metrics, error_reason_list)
        del logfile
        return [], None
...
def log_metrics(error_logs_count, is_ignored, ignored_reason, collected_metrics, error_reason_list):
    if error_logs_count > 0:
        collected_metrics['error_count'] += error_logs_count
    if is_ignored:
        logger.warning(
            f'Skipped S3 object because {ignored_reason}')
