...
class LogS3:
    ...
    def __init__(
        self, record, s3bucket, s3key, logtype, logconfig, s3_client, sqs_queue
    ):
    ...
    def set_rawfile_instance(self):
        if self.file_format == "text":
            ...
            return FileFormatWinEvtXml(self.rawdata, self.logconfig, self.logtype)
        elif self.file_format == "multiline":
            return FileFormatMultiline(self.rawdata, self.logconfig, self.logtype)
        elif self.file_format == "parquet":
            return FileFormatParquet(self.rawdata, self.logconfig, self.logtype)
    ...
    def logdata_generator(self) -> Tuple[str, dict, dict]:
        if (
            "eventSourceARN" in self.record
            and "kinesis" in self.record["eventSourceARN"]
        ):
            cwl_logmeta = {}
            ...
            logdict = self.rawfile_instacne.convert_lograw_to_dict(
                self.record["message"]
            )
    ...
    def set_start_end_position(self, ignore_header_line_number=None):
        if not ignore_header_line_number:
            ...
            ignore_header_line_number = self.rawfile_instacne.ignore_header_line_number
    ...
    def extract_file_timestamp(self):
        ...
        if re_file_timestamp_format:
            ...
            if m:
                ...
                msg = (
                    f"invalid file timestamp format regex, "
    ...
    def extract_firelens_log(self, start, end, logmeta={}):
        ...
        for logdata in self.rawdata.readlines()[start_index:end_index]:
            ...
            firelens_logmeta["ecs_task_definition"] = obj.get("ecs_task_definition")
            ...
            try:
                logdict = self.rawfile_instacne.convert_lograw_to_dict(logdata)
                if (
                    logdict == "regex_error"
        ...
        if mime_type not in ("text", "parquet"):
            ...
            if mime_type2 == "parquet":
                ...
                if mime_type == "gzip":
                    ...
                    z = zipfile.ZipFile(rawbody)
                    body = z.open(z.namelist()[0])
                    body = io.TextIOWrapper(body, encoding="utf8", errors="ignore")
                elif mime_type == "bzip2":
                    body = bz2.open(
                        rawbody, mode="rt", encoding="utf8", errors="ignore"
                    )
...
class LogParser:
    ...
    def __init__(
        self, logfile, logconfig, sf_module, geodb_instance, ioc_instance, xff_instance
    ):
    ...
    def add_basic_field(self):
        ...
        if (
            "eventSourceARN" in self.logmeta
            and "kinesis" in self.logmeta["eventSourceARN"]
        ):
            ...
            basic_dict["event"]["kinesis_arrival_timestamp"] = self.logmeta[
                "approximateArrivalTimestamp"
            ]
        else:
        ...
        if self.__skip_normalization:
            unique_text = f'{basic_dict["@message"]}{self.s3key}{self.additional_id}'
            ...
            basic_dict["@id"] = utils.value_from_nesteddict_by_dottedkey(
                self.__logdata_dict, self.logconfig["doc_id"]
            )
        ...
        if self.__skip_normalization:
            ...
            for field in self.logconfig["renamed_newfields"]:
                ...
                if v:
                    ...
                    utils.del_value_from_nesteddict(
                        self.logconfig[field], self.__logdata_dict
                    )
    ...
    def transform_to_ecs(self):
        ...
        if "ecs_task_arn" in self.logmeta:
            ...
            if "ec2_instance_id" in self.logmeta:
                ecs_dict["cloud"]["instance"] = {"id": self.logmeta["ec2_instance_id"]}
            ecs_dict["container"] = {
        ...
        if "__error_message" in self.logmeta:
            self.__logdata_dict["error"] = {"message": self.logmeta["__error_message"]}
    ...
    def transform_by_script(self):
        if self.logconfig["script_ecs"]:
            try:
                ...
                # self.logfile.error_logs_count += 1
                logger.exception(f"Exception error has occurs in sf_{self.logtype}.py")
    ...
    def enrich(self):
        ...
        if len(clientip_xff) == 2 and self.xff_instance.is_enabled:
            ...
            xff = utils.value_from_nesteddict_by_dottedkey(self.__logdata_dict, key_xff)
            if xff and len(xff) > 0:
                sourceip = utils.value_from_nesteddict_by_dottedkey(
                    self.__logdata_dict, key_clientip
                )
                clientip = self.xff_instance.select_clientip_from_xff(sourceip, xff)
                if clientip and (sourceip != clientip):
                    temp_dict = utils.put_value_into_nesteddict(key_clientip, clientip)
                    self.__logdata_dict = utils.merge_dicts(
        ...
        if ioc_ip_list and self.ioc_instance.is_enabled:
            ...
            for field in ioc_ip_list:
                ...
                if isinstance(ips, str):
                    ioc_ip_dict = self.create_ioc_ip_dict(ioc_ip_dict, ips, field)
                elif isinstance(ips, list):
                    for ip in ips:
                        ioc_ip_dict = self.create_ioc_ip_dict(ioc_ip_dict, ip, field)
    ...
    def add_field_prefix(self):
        if self.logconfig.get("field_prefix"):
            ...
            if field_prefix in self.__logdata_dict and isinstance(
                self.__logdata_dict[field_prefix], dict
            ):
                pass
            else:
            ...
            for field in self.original_fields:
                try:
                    self.__logdata_dict[field_prefix][field] = self.__logdata_dict[
                        field
                    ]
        ...
        for condition in exclusion_conditions:
            ...
            if is_excluded:
                ...
                if action == "count":
                    ...
                    logger.remove_keys(["condition_name", "expression", "log_record"])
    ...
    def get_timestamp(self):
        if (
            self.logconfig["timestamp_key"] or self.logconfig["timestamp_key_list"]
            ...
            dt = utils.convert_timestr_to_datetime_wrapper(
                timestr, timestamp_key, timestamp_format_list, self.timestamp_tz
            )
            if dt is None or dt == "":
...
def put_value_into_dict(key_str, v):
    ...
    if isinstance(v, dict):
        ...
        json_data = r'{{"{0}": "{1}" }}'.format(xkeys[-1], ",".join(map(str, v)))
    else:
